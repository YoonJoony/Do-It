...
# bridge : 다리에 진입한 트럭
bridge = deque([0] * (w + 1)) # w + 1개인 이유 : 배열을 앞으로 밀 때마다 맨 앞 배열에 넣을 트럭이 없을 경우 0으로 설정하기 위해.
time_cnt = 0
...
while queue or sum(bridge) != 0:
    i = 0 # i는 w를 카운트 하는 변수. (다음 차가 앞의 차 무게때문에 못들어올 경우 만들어줌)
          ...
          # 23번 줄 조건문으로 다음 트럭은 못들어오므로 i가 w미만이 될 때까지 그대로 다시 i에 1을 더해줌.
    while i < w:
        ...
        if queue and sum(bridge) + queue[0] <= L:
            if bridge and queue and bridge[0] != queue[0]:
                i = 0 # 새로운 트럭이 들어올 때마다 i의 값을 초기화.
            bridge[1] = queue.popleft()  # 다리에 트럭 진입.
